...
static sock_t open_listening_socket(union socket_address *sa) {
	...
	mg_open_msg = "";
	sock = socket(sa->sa.sa_family, SOCK_STREAM, IPPROTO_TCP);
	if (sock == INVALID_SOCKET) {
		mg_open_msg = "Cannot open socket";
		goto ret_err;
	}
#ifdef __WIN32__
	ret1 = setsockopt(sock, SOL_SOCKET, SO_EXCLUSIVEADDRUSE, (char *) &on, sizeof(on));
#else
	ret1 = setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (char *) &on, sizeof(on));
#endif
	if (ret1 != 0) {
		snprintf(message, sizeof(message), "set socket error %d:", ret1);
		mg_open_msg = message;
		closesocket(sock);
		sock = INVALID_SOCKET;
		goto ret_err;
	} else {
		snprintf(message, sizeof(message), "set socket: %d:", ret1);
		mg_open_msg = message;
	}
	...
	if (ret2 != 0) {
		snprintf(message, sizeof(message), "bind error %d:", ret2);
		mg_open_msg = message;
		closesocket(sock);
		sock = INVALID_SOCKET;
		goto ret_err;
	}
	ret3 = listen(sock, SOMAXCONN);
	if (ret3 != 0) {
		snprintf(message, sizeof(message), "listen error %d:", ret2);
		mg_open_msg = message;
		closesocket(sock);
		sock = INVALID_SOCKET;
		goto ret_err;
...
// everything passed
	...
	snprintf(message, sizeof(message),
		"Socket %d, options %d, bind %d, listen %d",
		sock, ret1, ret2, ret3);
	mg_open_msg = message;
...
int mg_set_option(struct mg_server *server, const char *name,
													const char *value, char* error_msg) {
	int ind = get_option_index(name);

	strcpy(error_msg,"No error");
	if (ind < 0) {
	strcpy(error_msg,"No such option");
	} else {
		...
		strcpy(error_msg, mg_open_msg);
			...
			if ((pw = getpwnam(value)) == NULL) {
				strcpy(error_msg, "Unknown user");
				return INVALID_SOCKET;
			} else if (setgid(pw->pw_gid) != 0) {
				strcpy(error_msg, "setgid() failed");
				return INVALID_SOCKET;
			} else if (setuid(pw->pw_uid) != 0) {
				strcpy(error_msg, "setuid() failed");
				return INVALID_SOCKET;
			}
			return 0;
#endif

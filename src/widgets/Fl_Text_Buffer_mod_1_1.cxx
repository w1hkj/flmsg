...
static Fl_Text_Buffer_mod* undowidget;
...
** will need to hold
*/
Fl_Text_Buffer_mod::Fl_Text_Buffer_mod( int requestedSize ) {
  mLength = 0;
...
** Free a text buffer
*/
Fl_Text_Buffer_mod::~Fl_Text_Buffer_mod() {
  free( mBuf );
  if ( mNModifyProcs != 0 ) {
    delete[] mNodifyProcs;
    delete[] mCbArgs;
...
*/
char * Fl_Text_Buffer_mod::text_range( int start, int end ) {
  char * s;
...
*/
char Fl_Text_Buffer_mod::character( int pos ) {
...
void Fl_Text_Buffer_mod::remove( int start, int end ) {
  const char * deletedText;
...
void Fl_Text_Buffer_mod::copy( Fl_Text_Buffer_mod *fromBuf, int fromStart,
...
*/
int Fl_Text_Buffer_mod::undo(int *cursorPos) {
...
*/
void Fl_Text_Buffer_mod::canUndo(char flag) {
  mCanUndo = flag;
}
...
*/
void Fl_Text_Buffer_mod::insert_column( int column, int startPos, const char *s,
  ...
  if ( nDeleted != insertDeleted )
    Fl::error("Fl_Text_Buffer_mod::insert_column(): internal consistency check ins1 failed");
...
void Fl_Text_Buffer_mod::overlay_rectangular( int startPos, int rectStart,
  ...
  if ( nDeleted != insertDeleted )
    Fl::error("Fl_Text_Buffer_mod::overlay_rectangle(): internal consistency check ovly1 failed");
  call_modify_callbacks( lineStartPos, nDeleted, nInserted, 0, deletedText );
  free( (void *) deletedText );
  if ( charsInserted != NULL )
    * charsInserted = nInserted;
...
void Fl_Text_Buffer_mod::replace_rectangular( int start, int end, int rectStart,
  ...
  if ( insertDeleted != deleteInserted + linesPadded )
    Fl::error("Fl_Text_Buffer_mod::replace_rectangular(): internal consistency check repl1 failed");
...
*/
void Fl_Text_Buffer_mod::tab_distance( int tabDist ) {
  const char * deletedText;
...
void Fl_Text_Buffer_mod::select( int start, int end ) {
...
void Fl_Text_Buffer_mod::unselect() {
  Fl_Text_Selection oldSelection = mPrimary;
...
void Fl_Text_Buffer_mod::remove_selection() {
  remove_selection_( &mPrimary );
}

void Fl_Text_Buffer_mod::replace_selection( const char *s ) {
  replace_selection_( &mPrimary, s );
}

void Fl_Text_Buffer_mod::secondary_select( int start, int end ) {
...
void Fl_Text_Buffer_mod::secondary_unselect() {
  Fl_Text_Selection oldSelection = mSecondary;
...
void Fl_Text_Buffer_mod::remove_secondary_selection() {
  remove_selection_( &mSecondary );
}

void Fl_Text_Buffer_mod::replace_secondary_selection( const char *s ) {
  replace_selection_( &mSecondary, s );
}

void Fl_Text_Buffer_mod::highlight( int start, int end ) {
...
void Fl_Text_Buffer_mod::unhighlight() {
  Fl_Text_Selection oldSelection = mHighlight;
...
*/
void Fl_Text_Buffer_mod::add_modify_callback( Fl_Text_Modify_Cb bufModifiedCB,
    void *cbArg ) {
...
void Fl_Text_Buffer_mod::remove_modify_callback( Fl_Text_Modify_Cb bufModifiedCB,
    void *cbArg ) {
  int i, toRemove = -1;
  Fl_Text_Modify_Cb *newModifyProcs;
  void **newCBArgs;
  ...
  if ( toRemove == -1 ) {
    Fl::error("Fl_Text_Buffer_mod::remove_modify_callback(): Can't find modify CB to remove");
    return;
  }
...
*/
void Fl_Text_Buffer_mod::add_predelete_callback(Fl_Text_Predelete_Cb bufPreDeleteCB,
	void *cbArg) {
...
void Fl_Text_Buffer_mod::remove_predelete_callback(
   Fl_Text_Predelete_Cb bufPreDeleteCB, void *cbArg) {
    ...
    if (toRemove == -1) {
    	Fl::error("Fl_Text_Buffer_mod::remove_predelete_callback(): Can't find pre-delete CB to remove");
    	return;
    }
...
*/
char * Fl_Text_Buffer_mod::line_text( int pos ) {
...
*/
int Fl_Text_Buffer_mod::line_start( int pos ) {
...
*/
int Fl_Text_Buffer_mod::line_end( int pos ) {
  if ( !findchar_forward( pos, '\n', &pos ) )
    pos = mLength;
  return pos;
}

int Fl_Text_Buffer_mod::word_start( int pos ) {
...
int Fl_Text_Buffer_mod::word_end( int pos ) {
...
** to ignore).
*/
int Fl_Text_Buffer_mod::character_width( char c, int indent, int tabDist, char nullSubsChar ) {
  /* Note, this code must parallel that in Fl_Text_Buffer_mod::ExpandCharacter */
  if ( c == '\t' )
...
*/
int Fl_Text_Buffer_mod::count_displayed_characters( int lineStartPos, int targetPos ) {
  int pos, charCount = 0;
...
*/
int Fl_Text_Buffer_mod::skip_displayed_characters( int lineStartPos, int nChars ) {
  int pos, charCount = 0;
  char c;
...
*/
int Fl_Text_Buffer_mod::count_lines( int startPos, int endPos ) {
...
*/
int Fl_Text_Buffer_mod::skip_lines( int startPos, int nLines ) {
  int pos, gapLen = mGapEnd - mGapStart;
  int lineCount = 0;
...
** the line
*/
int Fl_Text_Buffer_mod::rewind_lines( int startPos, int nLines ) {
  int pos, gapLen = mGapEnd - mGapStart;
  int lineCount = -1;
...
*/
int Fl_Text_Buffer_mod::insert_( int pos, const char *s ) {
...
*/
void Fl_Text_Buffer_mod::insert_column_( int column, int startPos, const char *insText,
...
static void insertColInLine( const char *line, char *insLine, int column, int insWidth,
  ...
  for ( linePtr = line; *linePtr != '\0'; linePtr++ ) {
    len = Fl_Text_Buffer_mod::character_width( *linePtr, indent, tabDist, nullSubsChar );
  ...
  if ( *insLine != '\0' ) {
    ...
    for ( c = retabbedStr; *c != '\0'; c++ ) {
      *outPtr++ = *c;
      len = Fl_Text_Buffer_mod::character_width( *c, indent, tabDist, nullSubsChar );
      indent += len;
    }
...
static void deleteRectFromLine( const char *line, int rectStart, int rectEnd,
  ...
  for ( ; *c != '\0' && indent < rectEnd; c++ )
    indent += Fl_Text_Buffer_mod::character_width( *c, indent, tabDist, nullSubsChar );
...
static void overlayRectInLine( const char *line, char *insLine, int rectStart,
  ...
  for ( linePtr = line; *linePtr != '\0'; linePtr++ ) {
    len = Fl_Text_Buffer_mod::character_width( *linePtr, inIndent, tabDist, nullSubsChar );
  ...
  for ( ; *linePtr != '\0'; linePtr++ ) {
    inIndent += Fl_Text_Buffer_mod::character_width( *linePtr, inIndent, tabDist, nullSubsChar );
    if ( inIndent >= rectEnd ) {
      linePtr++;
  ...
  if ( *insLine != '\0' ) {
    ...
    for ( c = retabbedStr; *c != '\0'; c++ ) {
      *outPtr++ = *c;
      len = Fl_Text_Buffer_mod::character_width( *c, outIndent, tabDist, nullSubsChar );
      outIndent += len;
    }
    free( (void *) retabbedStr );
  }
...
char * Fl_Text_Buffer_mod::selection_text_( Fl_Text_Selection *sel ) {
...
void Fl_Text_Buffer_mod::replace_selection_( Fl_Text_Selection *sel, const char *s ) {
  int start, end, isRect, rectStart, rectEnd;
...
*/
void Fl_Text_Buffer_mod::call_modify_callbacks( int pos, int nDeleted,
...
*/
void Fl_Text_Buffer_mod::call_predelete_callbacks(int pos, int nDeleted) {
    int i;
...
*/
void Fl_Text_Buffer_mod::redisplay_selection( Fl_Text_Selection *oldSelection,
...
void Fl_Text_Buffer_mod::move_gap( int pos ) {
...
*/
void Fl_Text_Buffer_mod::reallocate_with_gap( int newGapStart, int newGapLen ) {
  char * newBuf;
  int newGapEnd;
...
*/
void Fl_Text_Buffer_mod::update_selections( int pos, int nDeleted,
                                        int nInserted ) {
...
*/
int Fl_Text_Buffer_mod::findchar_forward( int startPos, char searchChar,
                                    int *foundPos ) {
  int pos, gapLen = mGapEnd - mGapStart;
...
** count lines quickly, hence searching for a single character: newline)
*/
int Fl_Text_Buffer_mod::findchar_backward( int startPos, char searchChar,
                                     int *foundPos ) {
  int pos, gapLen = mGapEnd - mGapStart;
...
static int textWidth( const char *text, int tabDist, char nullSubsChar ) {
  ...
  for ( c = text; *c != '\0'; c++ ) {
    if ( *c == '\n' ) {
      ...
      width += Fl_Text_Buffer_mod::character_width( *c, width, tabDist, nullSubsChar );
  }
  if ( width > maxWidth )
    return width;
  return maxWidth;
...
*/
void Fl_Text_Buffer_mod::rectangular_selection_boundaries( int lineStartPos,
  ...
  for ( pos = lineStartPos; pos < mLength; pos++ ) {
    ...
    if ( c == '\n' )
      break;
    width = Fl_Text_Buffer_mod::character_width( c, indent, mTabDist, mNullSubsChar );
...
static char *expandTabs( const char *text, int startIndent, int tabDist,
  ...
  for ( c = text; *c != '\0'; c++ ) {
    if ( *c == '\t' ) {
      len = Fl_Text_Buffer_mod::expand_character( *c, indent, outPtr, tabDist, nullSubsChar );
      outPtr += len;
      indent += len;
    } else if ( *c == '\n' ) {
      indent = startIndent;
...
static char *unexpandTabs( char *text, int startIndent, int tabDist,
  ...
  for ( c = text; *c != '\0'; ) {
    if ( *c == ' ' ) {
      len = Fl_Text_Buffer_mod::expand_character( '\t', indent, expandedChar, tabDist,
                                              nullSubsChar );
...
int
Fl_Text_Buffer_mod::insertfile(const char *file, int pos, int buflen) {
  FILE *fp;  int r;
...
int
Fl_Text_Buffer_mod::outputfile(const char *file, int start, int end, int buflen) {
  FILE *fp;
